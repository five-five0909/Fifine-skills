...
description: >
  当用户给出大型、复杂或耗时的任务时，结合 Trellis 任务生命周期与 Claude Code Agent Teams，
  自动完成：读取/生成 PRD → 注入 spec 约定 → 任务拆分 → 并行执行 → trellis-check 验证 → 写 journal
  归档。
  触发词：大任务、同时做、并行执行、agent team、任务拆分、批量处理、多模块同时、加速完成。
  只要任务涉及多个独立模块、多文件批量处理、或明显可并行的工作流就主动触发，不等用户说
  "用 agent team"。
  与 Trellis 的 trellis-brainstorm / trellis-before-dev / trellis-check / trellis-update-spec
  协同，不冲突。
---
