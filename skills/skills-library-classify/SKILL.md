---
name: skills-library-classify
description: Use this skill when the user needs to classify research PDF references into predefined topic buckets, especially for batch organization of literature folders with fallback confirmation on uncertain matches. Trigger: /skills-library-classify, classify references, 文献分类, PDF bucket.
---
...
## Trigger check
This skill applies when the user has a folder of research PDFs that need to be classified into topic buckets. If the user wants to rename files rather than classify them — stop, use skills-library-rename instead.
...
```bash
python <repo_root>/skills/skills-library-classify/scripts/scan_pdfs.py "<目标路径>"
```
...
```bash
python <repo_root>/skills/skills-library-classify/scripts/do_classify.py "<计划JSON路径>"
```
...
## 与 skills-library-rename 的联动

**skills-library-rename** 和 **ref-classify** 是同一工作流的两个阶段：

```
下载原始 PDF（任意文件名）
        ↓
   /skills-library-rename  →  YYYY_Title_Author.pdf（规范命名）
        ↓
  /skills-library-classify →  A-F 子目录（板块归档）
```
...
建议先运行 /skills-library-rename 完成重命名，再回来分类。
...
| 文件名不含年份（非规范命名） | 触发上方联动提示，让用户决定是否先跑 skills-library-rename |
...
- 脚本位置：`skills/skills-library-classify/scripts/`
