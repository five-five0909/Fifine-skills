---
name: academic-ref-classify
description: Use this skill when the user needs to classify research PDF references into predefined topic buckets, especially for batch organization of literature folders with fallback confirmation on uncertain matches. Trigger: /academic-ref-classify, classify references, 文献分类, PDF bucket.
---
...
```bash
python <repo_root>/skills/academic-ref-classify/scripts/scan_pdfs.py "<目标路径>"
```
...
```bash
python <repo_root>/skills/academic-ref-classify/scripts/do_classify.py "<计划JSON路径>"
```
...
下载原始 PDF（任意文件名）
   ...
   /ref-rename  →  YYYY_Title_Author.pdf（规范命名）
        ↓
  /academic-ref-classify →  A-F 子目录（板块归档）
```
...
- 脚本位置：`skills/academic-ref-classify/scripts/`
