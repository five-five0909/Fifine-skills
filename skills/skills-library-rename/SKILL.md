---
name: skills-library-rename
description: Use this skill when the user wants to batch rename research PDFs from extracted metadata, especially for incremental cleanup of literature libraries with user confirmation on naming choices. Trigger: /skills-library-rename, rename PDF, 文献重命名, batch rename.
---
...
## Trigger check
This skill applies when the user wants to batch rename research PDFs using extracted metadata. If the user wants to classify PDFs into topic folders — stop, use skills-library-classify instead.
...
- 脚本位置：`skills/skills-library-rename/scripts/`
...
```bash
python <repo_root>/skills/skills-library-rename/scripts/scan_refs.py "<用户路径>" --incremental -o <temp_scan.json>
```
...
```bash
python <repo_root>/skills/skills-library-rename/scripts/do_rename.py <plan.json> --dry-run
```
...
```bash
python <repo_root>/skills/skills-library-rename/scripts/do_rename.py <plan.json>
```
...
5. 执行后：
   ...
   - 若当前目录是 `plans/PISFM_Enhance/参考文献/`，**主动提示**：
     ```
     ✅ 重命名完成。如需将文件归入 A-F 板块子目录，可接着运行 /skills-library-classify。
     ```
