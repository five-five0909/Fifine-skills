...
\icmltitlerunning{research writing skill}
...
\begin{enumerate}
  ...
  \item \textbf{Template source manifest}: \texttt{templates/} records official sources, audit status, and pinned archive hashes without redistributing author kits whose licenses are unclear.
...
Table~\ref{tab:repository-evidence} summarizes local evidence used for this example paper. The repository includes audited venue source records, focused references for writing and submission workflows, deterministic helper scripts, and a portable single-entrypoint design centered on the root skill file.
...
The build workflow compiles the paper with the official ICML archive identified by URL and SHA-256 in \texttt{paper\_state.json}, then records warnings and source/PDF hashes. The resulting PDF remains in the repository so readers can inspect output quality directly, while the author-kit files themselves are fetched on demand rather than redistributed.
...
\section{Reproducible Contract Evaluation}

To make the example testable rather than purely descriptive, the repository includes a deterministic evaluation runner. It executes the failure-path regression suite, validates the skill package, checks citation-key integrity, and scans the paper package for unresolved markers. The runner records commands, exit codes, and output tails in a machine-readable JSON artifact and regenerates Table~\ref{tab:contract-evaluation} from those live results.

\input{tables/contract_evaluation.tex}

This evaluation has a deliberately narrow interpretation. A passing result shows that declared inputs resolve, tested failure modes are rejected, citation keys are internally consistent, and the checked package contains no unresolved markers. It does not measure factual correctness, novelty, reviewer preference, writing quality, or acceptance probability. Those outcomes require independent scientific and human evaluation.
...
This example paper has important limitations. Its contract evaluation measures mechanical reliability, not comparative writing quality. It does not benchmark the skill against other writing systems or provide user-study evidence. It does not prove that generated papers are correct, useful, or publishable. The repository inventory establishes only that certain files and workflows exist, not that they improve author productivity or paper quality.
...
Finally, official author instructions, page limits, checklist requirements, anonymity rules, archive hashes, and camera-ready policies can change. The source manifest makes changes visible, but any real submission must still verify the target venue's current instructions.
