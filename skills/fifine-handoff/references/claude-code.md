# Claude Code overlay — fifine-handoff skill

Load when executing `fifine-handoff` in **Claude Code**.

## Install location

- Personal (all projects): `~/.claude/skills/fifine-handoff/`
- Project (this repo only): `<repo>/.claude/skills/fifine-handoff/`

Claude Code auto-invokes by matching the `description` field (no `triggers:` needed), and `/fifine-handoff <goal>` works directly. Arguments after `/fifine-handoff` arrive as `$ARGUMENTS` / `$1`, but you should read the goal from the user's request rather than depending on substitution — that keeps the skill identical across harnesses.
...
|-------------------|--------------|
| Verify a change actually works | `fifine-grill-me-cn`, `fifine-paper-llm-research-grill` |
| Coordinate several workstreams | `fifine-parallel-executor-with-trellis` |
| Keep working from an older transcript | `fifine-trans-criptase` |
| Plan / pressure-test a multi-step feature | `fifine-grill-me-cn` |
| Write or polish the resulting report or paper | `fifine-research-paper-writing`, `fifine-english-research-write` |

The fifine-* namespace is distributed as plain skill folders, so the same names apply under `.codex/skills/`, `.agents/skills/`, or `.pi/skills/` if they are installed there.
...
Suggested first skill: <skill-name> — <why>.
```
