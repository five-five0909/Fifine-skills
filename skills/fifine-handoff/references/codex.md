# Codex CLI overlay — fifine-handoff skill

Load when executing `fifine-handoff` in **OpenAI Codex CLI** (skills system; the older custom-prompts feature is deprecated).

## Install location

- User-global: `~/.codex/skills/fifine-handoff/`
- Project: `<repo>/.codex/skills/fifine-handoff/`
- Shared cross-harness dir (also scanned by Codex): `~/.agents/skills/fifine-handoff/`
...
Prefer skills present in `~/.codex/skills/`, `.codex/skills/`, or `~/.agents/skills/`. Name the action in plain language if you cannot confirm a skill exists. Codex skills are folders with a `SKILL.md`; reference them by their `name`. Skills published from the fifine namespace keep their `fifine-` prefix (e.g. `fifine-grill-me-cn`, `fifine-trans-criptase`) — suggest those when they match the next-session goal and are present in one of the scanned directories.
