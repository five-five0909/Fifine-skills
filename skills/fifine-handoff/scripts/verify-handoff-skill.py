...
runs anywhere — no harness-specific install required, and no external dependency of any kind.
...
# Skill files must NOT issue external helper commands as part of the core flow.
# This Fifine version is dependency-free; overlays should not make soma/projectors part of execution.
SOMA_FREE_FILES = [
    "SKILL.md",
    "workflows/Handoff.md",
    "references/template.md",
...
def main() -> int:
    ...
    check("A2", "name: fifine-handoff" in fm, "frontmatter name: fifine-handoff")
    ...
    if desc_block:
        desc = desc_block.group(1)
    else:
        # Single-line (quoted) description — the portable form every harness parses.
        desc_line = re.search(r"^description:\s*(.+?)\s*$", skill, re.MULTILINE)
        desc = desc_line.group(1).strip().strip("'\"") if desc_line else ""
    ...
    check("A3", "/fifine-handoff" in desc, "description mentions /fifine-handoff")
    ...
    # A7: external helper CLIs are NOT dependencies — skill files issue no soma commands
