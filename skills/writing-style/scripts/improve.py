...
# 默认路径 — 自动检测通用 agents / Codex / Claude Code / standalone
def _detect_base():
    """检测数据存储基目录"""
    # 优先级: 环境变量 > ~/.agents > ~/.codex > ~/.claude > ~/.self-improving
...
def call_llm(prompt, timeout=180):
    """调用 LLM — 自动检测可用的 CLI (claude / llm)"""
    # 优先级: claude CLI → generic llm CLI → custom command
    candidates = [
