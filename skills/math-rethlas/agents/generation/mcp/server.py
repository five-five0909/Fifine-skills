...
# RETHLAS_MEMORY_ROOT lets the CLI redirect memory output to the user's output folder.
MEMORY_ROOT = Path(os.environ.get("RETHLAS_MEMORY_ROOT", str(REPO_ROOT / "memory")))
...
def verify_proof_service(
    ...
    """
    Record the verification result produced by Claude Code (the current session).
    Claude Code IS the verifier — it reads blueprint.md, checks the proof in-context,
    then calls this tool to persist the report to memory.

    verdict: 'correct' | 'incorrect' | 'incomplete'
    """
    if verdict not in ("correct", "incorrect", "incomplete"):
        raise ValueError("verdict must be 'correct', 'incorrect', or 'incomplete'")
    ...
    memory_append(problem_id, "verification_reports", report)
