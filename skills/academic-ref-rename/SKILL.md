---
name: academic-ref-rename
description: Use this skill when the user wants to batch rename research PDFs from extracted metadata, especially for incremental cleanup of literature libraries with user confirmation on naming choices. Trigger: /academic-ref-rename, rename PDF, 文献重命名, batch rename.
---
...
- 脚本位置：`skills/academic-ref-rename/scripts/`
...
```bash
python <repo_root>/skills/academic-ref-rename/scripts/scan_refs.py "<用户路径>" --incremental -o <temp_scan.json>
```
...
```bash
python <repo_root>/skills/academic-ref-rename/scripts/do_rename.py <plan.json> --dry-run
```
...
```bash
python <repo_root>/skills/academic-ref-rename/scripts/do_rename.py <plan.json>
```
