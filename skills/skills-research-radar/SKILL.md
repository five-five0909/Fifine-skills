...
安装后通常位于 `.claude/skills/skills-research-radar/scripts/daily-component-radar.mjs`（相对于消费者项目根目录）。
...
**示例命令**（`<skill-dir>` 替换为实际路径，如 `.claude/skills/skills-research-radar`）：

```bash
# 用户说"搜一下最近 Mamba 论文"
node .claude/skills/skills-research-radar/scripts/daily-component-radar.mjs --pack mamba-ssm --days 7 --max 30

# 用户说"找找 DeltaNet 和 xLSTM 的最新进展"
node .claude/skills/skills-research-radar/scripts/daily-component-radar.mjs --keywords "DeltaNet,xLSTM,linear recurrence" --days 14 --max 20

# 用户说"搜最近一个月高光谱的论文，多搜一些"
node .claude/skills/skills-research-radar/scripts/daily-component-radar.mjs --pack hyperspectral --days 30 --max 50

# 用户说"全部方向都跑一遍"
node .claude/skills/skills-research-radar/scripts/daily-component-radar.mjs --days 7 --max 20
```
