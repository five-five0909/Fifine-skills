...
name: tavily-search
description: |
  使用 Tavily MCP 进行实时网络搜索。每当用户想要搜索任何信息、查询最新资讯、查找文档、验证某个事实、了解某个技术的最新动态、搜索某个话题时，必须使用此 skill 调用 tavily-search 工具。
  ...
  - "帮我查找 X 的文档/教程/资料"
  - "网上有没有关于 X 的..."
  - 任何需要实时信息、当前状态、近期新闻的问题
  
  不要使用内置知识回答可能已过时的问题，直接调用 tavily-search。
compatibility:
  tools:
    - tavily-search  # 需要已安装 Tavily MCP
---
