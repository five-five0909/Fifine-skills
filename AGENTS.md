...
```json
{
  "include": ["fifine-lit-speed-read", "fifine-ref-classify"],
...
| Skill | Description |
|-------|-------------|
| fifine-dev-done-flow | 通用开发工作流引导工具，硬编码阶段序列 + required/optional 标记 + TODO 动态追踪 |
| fifine-rethlas | AI 驱动的数学公式形式化证明工具，驱动完整证明工作流 |
| fifine-idea-hook-forge | 论文 PDF 结构化解构工具，提取 hook 并输出 HTML 分析报告 |
...
| fifine-topic-refiner | 研究选题精炼工具 |
| fifine-ref-rename | 文献文件批量重命名 |
| fifine-ref-classify | 文献自动分类 |
| fifine-grill-me-cn | 方案压力测试工具 |
| fifine-llm-research-grill | LLM/PyTorch 研究方向自检 |
...
| fifine-trellis-task-orchestrator | Trellis 任务编排器 |
...
| 开发任务启动、规划工作流 | fifine-dev-done-flow |
| 方案压力测试、找逻辑漏洞 | fifine-grill-me-cn |
| 论文 PDF 结构化拆解、提取 hook | fifine-idea-hook-forge |
| 文献快速阅读、输出摘要报告 | fifine-lit-speed-read |
| LLM/PyTorch 研究方向审查 | fifine-llm-research-grill |
| 本地视频或音频转文稿 | fifine-media-transcript |
...
| PDF 文献自动分类到主题桶 | fifine-ref-classify |
| PDF 文献按元数据批量重命名 | fifine-ref-rename |
| 需要实时联网搜索当前信息 | fifine-tavily-search |
| 研究选题模糊，需要聚焦精炼 | fifine-topic-refiner |
| Trellis 任务编排、生成 PRD | fifine-trellis-task-orchestrator |
| 写稿/论文前的结构化自我审问 | fifine-write-research-grill |
