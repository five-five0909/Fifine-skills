...
description: >
  把用户的普通指令自动加工成带强烈情绪/脏话语气的版本，然后用加工后的 prompt 调用 AI 执行，
  返回结果给用户。用于提升 AI 遵循度，让 AI 更认真、更精确地执行指令。
  当用户说"帮我强化这个指令"、"加工一下"、"让AI认真执行"，或在指令前加 /amplify 前缀时触发。
  也可以作为其他 skill 的前置层使用。
---
